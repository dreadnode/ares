...
pub(super) fn tool_definitions() -> Vec<ToolDefinition> {
    vec![
        ...
        ToolDefinition {
            ...
            description: "Abuse Group Policy Object (GPO) permissions to perform privileged actions. Exploits write access to a GPO to add a local admin, create immediate tasks, or modify user rights on computers linked to the GPO.".into(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "gpo_name": {
                        "type": "string",
                        "description": "Display name or GUID of the target GPO"
                    },
                    "domain": {
                        "type": "string",
                        "description": "Target domain FQDN"
                    ...
                    "action": {
                        "type": "string",
                        "description": "Abuse action to perform (default: AddLocalAdmin). Options: AddLocalAdmin, AddComputerTask, AddUserRights",
                        "default": "AddLocalAdmin"
                    },
                    "user_to_add": {
                        "type": "string",
                        "description": "User to add as local admin or grant rights to (required for AddLocalAdmin and AddUserRights)"
                    },
                    "computer_target": {
                        "type": "string",
                        "description": "Specific computer to target within the GPO scope"
                    }
                },
        ...
        ToolDefinition {
            ...
            description: "Create an immediate scheduled task via GPO abuse using pyGPOAbuse. Exploits write access to a GPO to create a scheduled task that executes a command immediately on all computers within the GPO's scope. Useful for remote code execution across multiple hosts simultaneously.".into(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "gpo_name": {
                    ...
                    "command": {
                        "type": "string",
                        "description": "Command to execute via the scheduled task (e.g. 'net localgroup administrators attacker /add')"
                    },
                    "task_name": {
                        "type": "string",
                        "description": "Name of the scheduled task (default: WindowsUpdate)",
                ...
                "required": ["gpo_name", "domain", "username", "password", "dc_ip", "command"]
            }),
        },
        ToolDefinition {
            name: "dacl_edit".into(),
