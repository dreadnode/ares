...
@pytest.mark.asyncio
async def test_run_multi_agent_operation_waits_for_core_workers(monkeypatch):
    ...
    ensure_required_workers = AsyncMock()
    ...
    monkeypatch.setattr(orch, "_ensure_required_workers", ensure_required_workers)
    ...
    await run_multi_agent_operation(
        operation_id="op-core-workers",
        target_domain="contoso.local",
        target_ips=["192.168.58.2"],
        model="test-model",
    )

    ensure_required_workers.assert_awaited_once_with(
        dispatcher,
